...
export default function EnglishPodcastPage() {
  return (
    <InnerPage
      locale="en"
      hero={<PodcastHero locale="en" />}
    >
      <StructuredData data={createBreadcrumbJsonLd([{ name: "Home", path: "/en" }, { name: "Podcast", path: "/en/podcast" }])} />
      <section id="featured-podcast" className="py-24 md:py-32">
        <div className="page-shell">
          <SectionHeading eyebrow="Featured episode" description="Watch and listen directly from YouTube.">Latest <span className="gold-text">conversation</span></SectionHeading>
          <article className="overflow-hidden rounded-3xl border border-white/10 bg-[#0d0d0c] p-2 sm:p-3">
            <YouTubeEmbed videoId={podcastEpisodes[0].videoId} title={podcastEpisodes[0].titleEn} locale="en" loading="eager" className="rounded-[1.15rem]" />
            <div className="flex flex-col gap-4 px-4 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
              <div><span className="text-xs tracking-[0.15em] text-[#d6a642]">PODCAST · EPISODE 01</span><h3 className="mt-2 max-w-3xl text-xl font-medium leading-snug text-white">{podcastEpisodes[0].titleEn}</h3></div>
              <span className="inline-flex shrink-0 items-center gap-2 text-sm text-[#969289]"><Play className="size-4 fill-current text-[#d6a642]" /> Play video</span>
            </div>
          </article>
        </div>
      </section>
